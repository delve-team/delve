        ...
        for key in self.logs:
            ...
            for i, layer_name in enumerate(self.logs[key]):
                ...
                for stat in self.stats:
                    log_key = None
                    if stat == 'lsat':
                        log_key = key.replace(STATMAP['cov'],
                                              STATMAP['lsat']) + '_' + \
                                  layer_name
                        log_value = compute_saturation(cov_mat,
                                                       thresh=self.threshold)
                    elif stat == 'idim':
                        log_key = key.replace(STATMAP['cov'],
                                              STATMAP['idim']) + '_' + \
                                  layer_name
                        log_value = compute_intrinsic_dimensionality(
                                        cov_mat,
                                        thresh=self.threshold)
                    elif stat == 'cov':
                        log_key = key + '_' + layer_name
                        log_value = cov_mat.cpu().numpy()
                    if log_key is not None:
                        log_values[log_key] = log_value
