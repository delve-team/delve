...
def _filter_by_stat(paths: List[str],
                    stat: str,
                    neg: bool = False) -> List[str]:
...
def _filter_by_stat_shortcuts(paths: List[str],
                              stats: List[str],
                              neg: bool = False) -> List[str]:
    result = []
    for stat in stats:
...
def _sort_files_by_epoch(npy_files: List[str]) -> List[str]:
    npy_files.sort(
        key=lambda x: int(splitext(basename(x))[0].split('epoch')[-1]))
    return npy_files
...
def _obtain_stats_from_npy(npy_files: List[str]) -> pd.DataFrame:
    ...
    df_dict = {
        stat: _obtain_stat_from_npy(files, stat)
        for stat, files in grouped_files.items()
    }
    return pd.DataFrame.from_dict(df_dict)


def _recompute_value_from_cov(cov_path: str, stat: str,
                              thresh: float) -> float:
...
def _recompute_stat_from_cov(cov_name, cov_files: List[str], stat: str,
                             thresh: float) -> Dict[str, List[float]]:
...
def _recompute_stats(npy_files: List[str], stats: List[str],
    ...
    for cov_name, cov_paths in grouped_covariance.items():
        for stat in stats:
            stat_dict = _recompute_stat_from_cov(cov_name, cov_paths, stat,
                                                 thresh)
            df_dict.update(stat_dict)
    return pd.DataFrame.from_dict(df_dict)


def reconstruct_csv_from_npy_data(
    npywriter_out_path: str,
    ...
    if thresh is None:
        if 'sat' in stats and not _check_stat_available(
                npy_files, 'saturation'):
            warnings.warn(
                'No .npy file was found for saturation, stat will not be included'
            )
            stats.remove('sat')
        if 'idim' in stats and not _check_stat_available(
                npy_files, 'intrinsic-dimensionality'):
            warnings.warn(
                'No .npy file was found for intrinsic dimensionality, stat will not be included'
            )
            stats.remove('idim')
        ...
        if not _check_stat_available(npy_files, 'covariance'):
            raise FileNotFoundError(
                "Covariance Matrix was not found, recomputation impossible")
