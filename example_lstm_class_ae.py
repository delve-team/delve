...
test_loader = torch.utils.data.DataLoader(
    test_set, batch_size=batch_size, shuffle=False, num_workers=2, drop_last=True)
...
class NET(nn.Module):
    def __init__(self, in_dim = 784, hidden_dim = bs, n_layers = 1, out_dim =10 ,z_dim= 32):
        super(NET, self).__init__()
...
if __name__ == '__main__':
    ...
    stats = CheckLayerSat(savefile=logging_dir, save_to='plot', modules=net, include_conv=False, stats=['lsat'], max_samples=1024,
    ...
    for epoch in range(epochs):
        ...
        for i, (inputs, labels) in enumerate(loader):
            ...
            optimizer.zero_grad()
            outputs = net(inputs)
            loss = criterion(outputs.squeeze(), labels)  # output = [bs,out_dim],target= [bs]
